...
export function getInitialSchedule(
  ...
  if (!initial?.schedule) {
    return defaultKind === "one_time" ? { mode: "one_off", due_at: "" } : DEFAULT_SCHEDULE;
  }
  const s = initial.schedule;
  if (s.mode === "one_off") return { ...s };
  if (s.mode === "daily_times")
    return { mode: "daily_times", times: s.times ?? ["09:00"], amounts: s.amounts ?? [1] };
...
export function getInitialDuration(initial: MedRegimen | null | undefined): MedDuration {
  ...
  if (d.type === "until_date")
    return { type: "until_date", end_date: d.end_date ?? "", start_date: start };
...
export const mapLegacyIntakeAdvice = (v: string | null | undefined): string => {
  ...
  return (
    legacyToNew[v] ??
    (["before_meal", "with_meal", "after_meal", "before_bed", "morning_fasting", "custom"].includes(
      v,
    )
      ? v
      : "")
...
export function MedicationForm({
  ...
  const [intakeAdvice, setIntakeAdvice] = useState(
    () =>
      mapLegacyIntakeAdvice(initial?.intake_advice_type ?? null) ||
      (initial?.intake_advice_type === "none" ? "" : (initial?.intake_advice_type ?? "")),
  );
  ...
  const buildInventory = (): RegimenInventory | null =>
    inventoryEnabled
      ? {
          ...
          refill_threshold_amount: inventoryRefillThreshold
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const intake_advice_text =
      intake_advice_type === "custom" ? intakeAdviceCustom.trim() || null : null;
    ...
    >
      ...
      <div className={cn(isMobile && wizardStep !== 0 && "hidden")}>
        <section className={sectionWrapClass}>
                          ...
                          return (
                            ...
                            >
                              <Check
                                className={cn(
                                  "mr-2 h-4 w-4",
                                  selectedRegimenId === r.id ? "opacity-100" : "opacity-0",
                                )}
                              />
          ...
          <div
            className={cn("grid gap-4", isMobile ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2")}
          >
            <div className="space-y-2">
              ...
              <Select value={unit} onValueChange={(v) => setUnit(v as MedicationUnit)}>
                ...
                <SelectContent>
                  {[...MEDICATION_UNITS]
                    .sort((a, b) =>
                      t(medicationUnitKey(a), { count: 1 }).localeCompare(
                        t(medicationUnitKey(b), { count: 1 }),
                      ),
                    )
                    .map((u) => {
          ...
          <section className={sectionWrapClass}>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => {
                  if (v === "daily_times")
                    setScheduleMode({ mode: "daily_times", times: ["09:00"], amounts: [1] });
                  else if (v === "interval_hours")
                    setScheduleMode({
                      mode: "interval_hours",
                      interval: {
                        every:
                          schedule.mode === "interval_hours"
                            ? ((schedule as { interval?: { every?: number } }).interval?.every ?? 1)
                            : 1,
                      },
                      amount:
                        schedule.mode === "interval_hours"
                    ...
                    setScheduleMode({
                      mode: "interval_days",
                      interval: { every: prev?.interval?.every ?? 1 },
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="days_of_week">
                    {t("medications.frequencyDaysOfWeek")}
                  </SelectItem>
                </SelectContent>
              </Select>
                      ...
                      onValueChange={(n) => {
                        ...
                        if (schedule.mode === "interval_hours")
                          setSchedule({
                            ...schedule,
                            interval: { every },
                            amount: (schedule as { amount?: number }).amount ?? 1,
                          });
                        else
                          setSchedule({
                            ...schedule,
                            interval: { every },
                            times: (schedule as { times?: string[] }).times ?? ["09:00"],
                            amounts: (schedule as { amounts?: number[] }).amounts ?? [1],
                          });
                      }}
                      className="w-24"
                    />
              ...
              {schedule.mode === "days_of_week" && (
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {([0, 1, 2, 3, 4, 5, 6] as const).map((d) => {
                    const isSelected =
                      (schedule as { days_of_week?: number[] }).days_of_week?.includes(d) ?? false;
                    return (
                      <Toggle
                        key={d}
                        variant="outline"
                ...
                <Select
                  ...
                  onValueChange={(v) => {
                    ...
                    if (v === "endless") setDuration({ type: "endless", start_date: startDate });
                      ...
                      setDuration({
                        type: "until_date",
                        end_date: duration.type === "until_date" ? duration.end_date : "",
                        start_date: startDate,
                      });
                    else
                      setDuration({
                        type: "for_days",
                        days: duration.type === "for_days" ? duration.days : 1,
                        start_date: startDate,
                      });
                  }}
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="days_from_start">
                      {t("medications.endTypeDaysFromStart")}
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="__none__">{t("medications.intakeAdviceNone")}</SelectItem>
                  {INTAKE_ADVICE_OPTIONS.map((a) => (
                    <SelectItem key={a} value={a}>
                      {t(
                        `medications.intakeAdvice${
                          a === "custom"
                            ? "Custom"
                            : a
                                .split("_")
      ...
      {isMobile ? (
        <div className="fixed left-0 right-0 z-[70] flex gap-2 flex-wrap items-center p-4 bg-background border-t shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] dark:shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.3)] bottom-[calc(3.5rem+max(0.5rem,env(safe-area-inset-bottom)))]">
          {currentStep > 0 ? (
