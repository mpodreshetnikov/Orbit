...
function CustomDot(props: { cx?: number; cy?: number; payload?: { hasDuplicate?: boolean } }) {
  ...
  return (
    <g>
      ...
      />
      {hasDuplicate && (
        <text x={cx} y={cy - 10} textAnchor="middle" fill="#f97316" fontSize={10} fontWeight="bold">
          !
        </text>
      )}
    </g>
        ...
        <thead className="bg-muted/50">
          <tr>
            ...
            <th className="text-left p-2 sm:p-3 font-medium">
              {t("observationHistory.reference")}
            </th>
...
function convertFromCanonical(
  ...
  targetUnit: string,
  acceptedUnits: Record<
    string,
    { factor_to_canonical?: number; formula_to_canonical?: string }
  > | null,
): number | null {
...
export default function ObservationDetailPage({
  ...
  const {
    ...
    isLoading,
    error,
  } = useSingleObservationHistory(selectedPersonId, decodedObsCode);
        ...
        {availableUnits.length > 1 && (
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            <span className="text-xs sm:text-sm text-muted-foreground hidden sm:inline">
              {t("observations.displayUnit")}:
            </span>
            <Select value={selectedUnit || canonicalUnit} onValueChange={setSelectedUnit}>
      ...
      {observation && (
        <>
                  ...
                  observation.latest_status !== "unknown" && (
                    <div className="flex items-center gap-1 mt-1 sm:mt-2 text-orange-600">
                      ...
                      <span className="text-xs sm:text-sm">
                        {t("observationHistory.outOfRange")}
                      </span>
                    </div>
                  )}
              </CardContent>
              ...
              <CardContent className="p-3 pt-0 sm:p-6 sm:pt-0">
                {displayRefLow !== null && displayRefHigh !== null ? (
                  <div className="text-sm sm:text-lg font-semibold">
                    {displayRefLow.toFixed(displayRefLow % 1 === 0 ? 0 : 1)} –{" "}
                    {displayRefHigh.toFixed(displayRefHigh % 1 === 0 ? 0 : 1)}
                    {displayUnit && (
          ...
          <Card>
            <CardHeader className="p-3 sm:p-6">
              <CardTitle className="text-sm sm:text-base">
                {t("observationHistory.chartTitle")}
              </CardTitle>
            </CardHeader>
          ...
          <Card>
            <CardHeader className="p-3 sm:p-6">
              <CardTitle className="text-sm sm:text-base">
                {t("observationHistory.historyTitle")}
              </CardTitle>
            </CardHeader>
