---
id: T-0004
title: Money module (budget and investments) technical specification
status: open
kind: feature
priority: p2
depth: note
...
updated: 2026-03-02
owner: Max
tags: [money, budget, investments, spec]
---
...
This is a standing technical specification for the money module, not a plan for a single sitting of
work. It describes the intended domain model, import pipeline, categorization behaviour and UI for
Budget (phase 1) and Investments (phase 2), and it is the reference that individual money tasks are
cut from.

It carries `depth: note` rather than `depth: execplan` because its body is a specification rather
than an executable plan: it states the target design, not an ordered set of milestones a novice
could follow start to finish. Work derived from it is tracked as separate tasks, each of which may
be an ExecPlan in its own right.

The full specification is preserved verbatim below, beginning at `## 0) Goals`.

## Progress

Progress on this specification is tracked by the tasks cut from it rather than in this file, because
the specification describes a target state that several independent pieces of work move toward.

- [x] (2026-03-14) Canonical categories and the rule pipeline delivered — see `T-0001`.
- [ ] Remaining phase 1 (Budget) scope per the specification below.
- [ ] Phase 2 (Investments & Savings) not started.

## Decision Log

- Decision: Keep this document as a `note`-depth specification instead of converting it into an
  ExecPlan.
  Rationale: `docs/PLANS.md` requires an ExecPlan to be followable end to end by a novice holding
  only the working tree and that one file. This document is a design reference spanning two phases
  and does not have that shape. Forcing it into the ExecPlan skeleton would have meant either
  rewriting the specification or filling the mandatory sections with placeholder text, and both lose
  information. Tasks cut from it can be ExecPlans where the work warrants one.
