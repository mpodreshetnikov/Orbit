...
- A pull-request verdict expires. "My version sorts after everything on the base" stops being true when somebody else merges, with nothing on the branch moving, and no `pull_request` event fires for that. The binding run is therefore the merge queue's: `main.yml` also runs on `merge_group`, where GitHub re-runs the required checks against the projected merged result. **The queue has to be enabled in branch protection for `main` for that to happen**; without it the workflow's `merge_group` trigger never fires and the gate is back to judging an expired state.
