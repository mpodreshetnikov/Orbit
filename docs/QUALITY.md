...
- `supabase/migrations/.out-of-order-allowlist` is the reviewed exception for ordering only. Each entry is `<migration filename> # <why it is safe against the newer schema>`; the rationale is required and an entry without one fails the check. The key is the filename rather than the bare version, so an exemption cannot be inherited by a different file that reuses the timestamp -- production would skip that file's SQL entirely, the version already being in `schema_migrations`.
