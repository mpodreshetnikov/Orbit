...
| `ARCH-G06` | Documentation map drift in rules/plans locations — resolved 2026-08-13 | Slows onboarding and creates ambiguity on canonical planning paths                          | Was: `AGENTS.md` referenced `docs/PLANS.md/` while planning content was split across `docs/PLANS.md` and `docs/exec-plans/`. Resolved by unifying all work into a single task registry, canonical for both where work lives and how a plan is written, with a validator gating its layout. The registry moved to its own private repository when this one was published; `AGENTS.md` routes to it. |
