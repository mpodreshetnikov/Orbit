...
- A new migration's timestamp must sort **after** every migration already on the base branch, and the check asks that of every migration in the tree rather than only of the ones you add. Canonical policy, including the allowlisted exception and why a rename stops being safe once the version is applied, is `docs/QUALITY.md` under "Migration Order Policy"; `quality-migration-order` (part of `quality`) enforces it.
