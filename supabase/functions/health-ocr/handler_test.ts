...
Deno.test("health-ocr handler responds to OPTIONS", async () => {
  ...
  const response = await handleRequest(new Request("http://localhost/functions/v1/health-ocr", { method: "OPTIONS" }));
  assertEquals(response.status, 200);
  assertEquals(response.headers.get("Access-Control-Allow-Origin"), corsHeaders["Access-Control-Allow-Origin"]);
});

Deno.test("health-ocr handler returns env configuration error when OPENROUTER key is missing", async () => {
