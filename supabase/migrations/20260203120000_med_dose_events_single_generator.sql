-- ============================================================================
-- Single reusable dose-event generator + interval_hours + full interval_days
-- ============================================================================
-- 1) One core function generate_med_dose_events_for_person_ids used by both
--    cron (generate_med_dose_events_for_horizon) and post create/edit
--    (generate_med_dose_events_for_horizon_for_person). Ensures consistent
--    behavior everywhere.
-- 2) Adds interval_hours event generation (every N hours).
-- 3) Full schedule for interval_days (times[] + amounts[], slot_time alias).
-- ============================================================================
...
COMMENT ON FUNCTION public.generate_med_dose_events_for_person_ids(uuid[], text, int) IS
  'Single reusable generator: med_dose_events for given person ids over horizon. Supports one_off, interval_hours, daily_times, days_of_week, interval_days (full times/amounts). Only active, non-deleted regimens. Used by cron and by post create/edit.';

-- Cron entry point: resolve auth user persons and delegate
...
-- Post create/edit entry point: delegate for one person
