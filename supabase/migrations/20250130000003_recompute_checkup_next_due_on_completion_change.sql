-- ============================================================================
-- Recompute checkup next_due_at when completion history is updated or deleted,
-- or when checkup_items.schedule is updated (frequency / first due date).
-- ============================================================================

-- Recompute next_due_at (and status for one_off) for a checkup item based on
-- its current completions. Used after UPDATE or DELETE on checkup_completions.
...
-- After UPDATE on a completion (e.g. done_at changed), recompute item's next_due_at
...
-- After DELETE of a completion, recompute item's next_due_at (or restore one_off to active)
...
-- After UPDATE on checkup_items (e.g. schedule / frequency / first due date changed), recompute next_due_at
