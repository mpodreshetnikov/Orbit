...
export function createInitialBackfillState(
  ...
  return { cursorMs: null, horizonMonths, completedAtMs: null };
}
...
export function planIncrementalWindow(
  ...
  return {
    windowFromIso: new Date(nowMs - spanMs).toISOString(),
...
export interface BackfillSliceOutcome {
  ...
  ok: boolean;
  /** Receipts the run left for next time. */
...
export function shouldAdvanceBackfillCursor(outcome: BackfillSliceOutcome): boolean {
  return outcome.ok && outcome.skippedAfterBudgetCount === 0 && !outcome.partial;
}
